...
import MakeSelections from "../admin/fmb/make-selections/make-selections"
...
const AdminMenu = ({
  ...
  const handleMenuOpenClose = (value: any) => {
      ...
      >
        ...
        {currUser.permissions.fmb && (
          <SubMenu key="fmb" title="Faiz-ul-Mawaid">
            ...
            <Menu.Item key="fmb-make-selections">
              Make Selections for Families
            </Menu.Item>
          ...
          <Col xs={10}>
            <Form initialValues={{ mobilemenuitem: currMenuItem }}>
              <Form.Item name="mobilemenuitem">
                <Select
                  style={{ width: "100%" }}
                  onChange={(e: any) => handleChangePageMobile(e)}
                >
                  {currUser.permissions.manage_user_accounts && (
                  ...
                  {currUser.permissions.fmb && (
                    <OptGroup label="Faiz-ul-Mawaid">
                      ...
                      <Option value="fmb-make-selections">
                        Make Selections for Families
                      </Option>
...
const Admin = () => {
  ...
  const handleChangePageMobile = (value: any) => {
    setPage(value)
  }
  ...
  const getMenus = async () => {
    if (shouldFetchMenusFromFirebase) {
      ...
      try {
        const queryForFmbHijriDocRef = doc(db, "fmb", hijriYear.toString())
        ...
        if (yearCollection.exists()) {
          const menusFromFirebase = await getDocs(
            collection(db, "fmb", hijriYear.toString(), "menus")
          )

          menusFromFirebase.forEach((docSnap: any) => {
            let formattedDocData = {
              ...docSnap.data(),
              year: docSnap.id === "moharram" ? hijriYear + 1 : hijriYear,
              month: docSnap.id,
